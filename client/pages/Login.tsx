    ...
    <div className="min-h-screen bg-brand-base pt-32 pb-24 flex items-center justify-center px-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-12">
          <span className="text-brand-terracotta uppercase tracking-[0.3em] text-[10px] mb-4 block font-bold">Welcome Back</span>
          <h1 className="text-4xl font-serif text-brand-dark italic mb-4">Sign In</h1>
        ...
        <div className="bg-white p-10 shadow-2xl border border-brand-peach/50 rounded-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              {isLoading ? 'Verifying...' : 'Sign In'} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
            </button>
          </form>

          <div className="my-8 flex items-center gap-4">
            <div className="flex-1 h-[1px] bg-brand-peach"></div>
            <span className="text-[10px] uppercase tracking-luxury text-brand-muted font-bold">Or</span>
            <div className="flex-1 h-[1px] bg-brand-peach"></div>
          </div>

          <button
            disabled={isLoading}
            className="w-full border border-brand-brown/20 py-4 uppercase text-[10px] font-bold tracking-luxury hover:bg-brand-base transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50"
          >
            <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" className="w-4 h-4" alt="Google" />
            {isLoading ? 'Connecting...' : 'Sign in with Google (Coming Soon)'}
          </button>
        </div>

        <p className="mt-8 text-center text-xs text-brand-muted font-light tracking-wide">
          Don't have an account? {' '}
          <Link to="/signup" className="text-brand-terracotta font-bold hover:underline">Register here</Link>
        </p>
      </div>
    </div>
  );
