...
const Checkout: React.FC = () => {
  ...
  return (
    <div className="bg-brand-base min-h-screen pt-28 pb-24">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  <label className="block text-[10px] uppercase tracking-wide text-brand-muted mb-2 font-bold">Street Address</label>
                  <input required type="text" name="address" value={formData.address} onChange={handleChange} className={inputClasses} placeholder="House no., building, street, area" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input required type="text" name="pincode" value={formData.pincode} onChange={handleChange} className={inputClasses} pattern="[0-9]{6}" placeholder="6-digit PIN" />
                </div>
                <div>
                  <label className="block text-[10px] uppercase tracking-wide text-brand-muted mb-2 font-bold">Phone</label>
                  <input required type="tel" name="phone" value={formData.phone} onChange={handleChange} className={inputClasses} placeholder="+91 XXXXXXXXXX" />
                </div>
              </div>
            </section>
      ...
      {showRazorpayMock && currentGatewayOrder && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-[#2D2E32] w-full max-w-md rounded-lg overflow-hidden shadow-2xl">
            <div className="bg-[#3A3C42] p-6 flex justify-between items-center border-b border-white/5">
              ...
              <button
                onClick={() => { setShowRazorpayMock(false); setIsProcessing(false); }}
                className="text-white/40 hover:text-white/80 transition-colors text-xl font-light"
              >
                ✕
              </button>
            </div>
            ...
            <div className="bg-[#1E1F21] p-4 text-center">
              <p className="text-white/30 text-[9px] uppercase tracking-[0.2em] font-bold">Secure checkout — Demo Mode</p>
            </div>
          </div>
        </div>
      )}
